...
class MyBagOfWords(object):
	...
	def run(self):
		if self.source == "MSH":
			...
			for lemmatized_words, label in zip(self.Data, self.Labels):
				print(lemmatized_words)
				self.processAbstract(lemmatized_words, label)
			output = open(self.DictFil, 'wb')
			pickle.dump(self.word2key, output)
			output.close()
	...
	def preprocessWords(self, abstract):
		...
		#abstract = abstract.translate(str.maketrans(string.punctuation, ' ')) # remove punctuations
		tokens = [token.lower() for token in word_tokenize(abstract)] # lowercase and tokenize 
		lemmatized_words = [wnl.lemmatize(token) for token in tokens] # limmatize
		lemmatized_words = [w for w in lemmatized_words if not w in stop_words] # remove stop words
	...
	def processAbstract(self, lemmatized_words, label):
		lemmatized_words = [word for word in lemmatized_words if word in self.KeptWords]
		try:
		...
		word_array = np.array(word_array, dtype='float64')
		"""
		word_window = lemmatized_words[max(0, idx-10) : min(idx + 10, len(lemmatized_words))]
	...
	def processAbstract2(self, lemmatized_words):
		lemmatized_words = lemmatized_words = [word for word in lemmatized_words[0] if word in self.KeptWords]
		#print(lemmatized_words)
		word_array = []
		try:
			idx = lemmatized_words.index(self.the_word)
