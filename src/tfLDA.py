...
flags.DEFINE_float(
    "learning_rate", default=5e-3, help="Learning rate.")  # 3
flags.DEFINE_integer(
    "max_steps", default=5000, help="Number of training steps to run.")
flags.DEFINE_integer(
    "num_topics",
    default=2,
...
flags.DEFINE_string(
    "master_data_dir",
    default="/Users/linyingzhang/git/zhangly811/WordSenseDetection/dat_unsync/output/",  # "/tmp/lda/data"
...
flags.DEFINE_integer(
    "viz_steps", default=2999, help="Frequency at which save visualizations.")
...
def model_fn(features, labels, mode, params, config):
  ...
  """
    ...
    # Obtain the topic assigned to each document. Implemented as a py_func for simplicity.
    doc_labels = tf.compat.v1.py_func(
        get_doc_labels,
        [topics_posterior.concentration],
        tf.string,
        stateful=False)
    tf.compat.v1.summary.text("doc_labels", doc_labels)
    ...
    return tf.estimator.EstimatorSpec(
        ...
        eval_metric_ops={
            ...
            "topic_per_doc": (doc_labels, tf.no_op()),
        },
    )


def get_doc_labels(topics_posterior_concentrations):
  ...
  """
    topic_per_doc = np.argmax(topics_posterior_concentrations, axis=1)
    # np.savetxt(os.path.join(model_dir, 'doc_labels.txt'), topic_per_doc, fmt='%i', delimiter=",")
    return topic_per_doc.astype(str)


def get_topics_strings(topics_words, alpha, vocabulary,
                       topics_to_print=10, words_per_topic=10):
    """Returns the topic assigned to each document.
...
def build_input_fns(data_dir, batch_size):
  ...
  """
    ...
    def train_input_fn():
        dataset_train = newsgroups_dataset(
            data_dir, "train", num_words, shuffle_and_repeat=False)
        dataset_test = newsgroups_dataset(
        ...
        # dataset = dataset.batch(num_documents)#.prefetch(32)
        # return tf.compat.v1.data.make_one_shot_iterator(dataset).get_next()
        return dataset
    ...
    def eval_input_fn():
        dataset = newsgroups_dataset(
            data_dir, "test", num_words, shuffle_and_repeat=False)
        # dataset = dataset.batch(num_documents)
        # return tf.compat.v1.data.make_one_shot_iterator(dataset).get_next()
        return dataset
...
def main(argv):
    ...
    # if FLAGS.source == "MSH":
    #     LABEL_FILE_TEMPLATE = "{split}.labels"
    #     labels = np.load(data_dir, LABEL_FILE_TEMPLATE.format(split=split_name), allow_pickle=True)
    ...
    for step in range(FLAGS.max_steps // FLAGS.viz_steps):
